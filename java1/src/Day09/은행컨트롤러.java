...
public class 은행컨트롤러 { // 클래스 
	...
	public boolean 입금( String 계좌번호 , int 입금액) { 
		...
		for( 은행 temp : 은행계좌프로그램_상속.계좌리스트  ) {
			if( temp !=null && temp.get계좌번호().equals(계좌번호) ) {
				...
				은행계좌프로그램_상속.계좌리스트[i].set예금액( temp.get예금액()+입금액 );
										// Setter	= 기존금액 + 새로운 입금액
				return true;
			}
			i++; 
	...
	public int 출금( String 계좌번호 , String 비밀번호 , int 출금액  ) {
		...
		int i = 0 ;
		for( 은행 temp : 은행계좌프로그램_상속.계좌리스트 ) { // 1. 동일한 계좌번호를 찾아서
			if( temp !=null && temp.get계좌번호().equals(계좌번호)  // 2. 해당 계좌번호와 비밀번호가 일치 확인 
				...
				if( temp.get예금액() < 출금액 ) {
					...
					// 3. 비밀번호가 일치하면 출금처리 
					은행계좌프로그램_상속.계좌리스트[i].set예금액(temp.get예금액()-출금액);
	...
	public int 이체( String 계좌번호 , String 비밀번호 , int 이체금액 , String 받는계좌 ) {
		...
		for( 은행 temp : 은행계좌프로그램_상속.계좌리스트 ) { // 본인 = 보내는사람 계좌는 temp
			if( temp != null && temp.get계좌번호().equals(계좌번호) &&  
					temp.get비밀번호().equals(비밀번호) ) {
				int j = 0; ; // 받는사람계좌 인덱스 위치 변수
				for( 은행 temp2 : 은행계좌프로그램_상속.계좌리스트 ) { // 받는 사람 계좌는 temp2 
					if( temp2 !=null & temp2.get계좌번호().equals(받는계좌) ) {
						
						if( temp.get예금액() < 이체금액 ) {
							return 1;// 잔액부족 
						}else {
							은행계좌프로그램_상속.계좌리스트[i].set예금액( temp.get예금액() - 이체금액 );
							은행계좌프로그램_상속.계좌리스트[j].set예금액( temp2.get예금액() + 이체금액 );
							return 2;
							// 성공 
						}
					}

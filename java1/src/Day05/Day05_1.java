...
public class Day05_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("1차원 배열" + 배열); // 인덱스없이 배열명만 호출시
		System.out.println("2차원 배열" + 배열2); // [메모리]주소값 출력
		
		
		System.out.println("1차원 배열" + 배열.length); // 배열의 길이
		System.out.println("2차원 배열 행 길이" + 배열2.length); // 2차원 배열의 행길이
		System.out.println("2차원 배열 열 길이" + 배열2[0].length); // 2차원 배열의 행길이
		
		// 예2) 반복문을 이용한 0~49까지 첫번째 행에 저장
				//50~99까지 두번쨰 행에 저장하는 2차원 배열 선언
		int[][] 배열3 = new int[2][50]; // int : 4바이트 *2*50 -> 400
		
		for( int i =0; i<100; i++) {
				// i는 0부터 100미만까지 1씩증가 실행 반복처리 -> 총100회 
			
				// 첫번째 행 값 넣기
			if(i<50)  배열3[0][i] = i;
			
			else 배열3[1][i-50] = i; 
			// i가 50 이상이면 두번째행 i에 50을 - 한이유는 []상자가	
			// 50을 넘으면 안되기 때문에 조건에 -50을 해서 저장할 수 있게 바꾼것이다
				//[0] : 첫번쨰 행번호
				//[i] : i번쨰 열번호
		}
